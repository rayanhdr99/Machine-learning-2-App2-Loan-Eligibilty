# app.py - main streamlit app for the loan eligibility predictor
import logging
import os
import sys
...
# make sure python can find our src package in the same folder
sys.path.insert(0, os.path.dirname(__file__))

# import our custom modules for loading data, preprocessing, training, and evaluating
...
# path to the csv dataset file in the data folder
DATA_PATH = os.path.join(os.path.dirname(__file__), "data", "credit.csv")

# configure the streamlit page title, icon, and layout
st.set_page_config(page_title="Loan Eligibility Predictor", page_icon="\U0001f4b0", layout="wide")


@st.cache_data
def get_data():
    # load and cache the dataset so we don't re-read the csv on every rerun
    return load_data(DATA_PATH)


@st.cache_resource
def get_models():
    # this function runs the full pipeline: load -> impute -> encode -> split -> train
    df = get_data()
    df = impute_missing(df)  # fill in missing values
    df_encoded = encode_and_prepare(df)  # one-hot encode categorical columns
    X_train_s, X_test_s, y_train, y_test, scaler, feature_cols = split_and_scale(df_encoded)  # split and scale
...
def main():
    ...
    st.title("\U0001f4b0 Loan Eligibility Predictor")
    ...
    # try loading data and training models, show error if something goes wrong
    try:
        df_raw = get_data()
    ...
    if page == "Dataset Overview":
        ...
        # bar chart showing how many approved vs denied
        ...
        # check for any remaining missing values in the raw data
    ...
    # ---- PAGE 2: Model Performance ----
        ...
        # evaluate all three models and display accuracy for each
    ...
    # ---- PAGE 3: Predict Eligibility ----
        ...
        # input form split into two columns for a cleaner layout
        ...
        # let the user pick which model to use for prediction
        ...
        if st.button("Check Eligibility", type="primary"):
            try:
                # build a dictionary with all the input features, one-hot encoded manually
                input_dict = {
                ...
                # make sure input columns match the training columns exactly
                ...
                # pick the selected model and make a prediction
                ...
                # show prediction confidence if the model supports probabilities
